...
from odoo.tools import file_open  # pyright: ignore[reportMissingImports]


def _setup_module(env):
    if env.ref('base.main_company', False):
        # with file_open('dealer/static/src/img/favicon.ico', 'rb') as file:
        #     env.ref('base.main_company').write({
        #         'favicon': base64.b64encode(file.read())
        #     })
        with file_open('muk_web_theme/static/src/img/ps-logo.png', 'rb') as file:
             env.ref('base.main_company').write({
